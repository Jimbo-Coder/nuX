...
extern "C" void nuX_M1_CalcUpdate(CCTK_ARGUMENTS) {
  ...
  // per-thread state for limiter/application kernels small and lets cheap
  // source regimes avoid the implicit solver call path entirely.
  grid.loop_int_device<1, 1, 1>(
      ...
      [=] CCTK_DEVICE(const PointDesc &p) {
        ...
        for (int ig = 0; ig < groupspec; ++ig) {
          ...
          if (source_rates_are_zero(eta_0[i4D], eta_1[i4D], abs_0[i4D],
          ...
          int const source_status =
              source_is_nonstiff(dt, abs_1[i4D], scat_1[i4D])
                  ? source_update_nonstiff(source_ctx, closure_fun, &chi[i4D],
                                           &Enew, &Fnew_d)
              : source_uses_thick_limit(dt, abs_1[i4D], scat_1[i4D],
                                        source_thick_limit)
                  ? NUX_M1_SOURCE_EQUIL
              : source_uses_scat_limit(dt, scat_1[i4D], source_scat_limit)
                  ? NUX_M1_SOURCE_SCAT
                  : source_update(source_ctx, closure_fun, &chi[i4D], &Enew,
