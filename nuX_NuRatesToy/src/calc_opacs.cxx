...
extern "C" void nuX_NuRatesToy_Calc(CCTK_ARGUMENTS) {
  ...
  const auto calcrates =
      [=] CCTK_DEVICE(const MyQuadrature *quad_1d, const MyQuadrature *quad_2d,
        ...
        case rates_t::NuRates: {
          ratesout =
              ComputeM1Opacities(quad_1d, quad_2d, my_grey_opacity_params);
          break;
        }
      ...
      [=] CCTK_DEVICE(const PointDesc &p) CCTK_ATTRIBUTE_ALWAYS_INLINE {
        ...
        for (int ig = 0; ig < ngroups * nspecies; ++ig) {
          ...
          my_grey_opacity_params.m1_pars.J[ig] =
              rJt[i4D] * nuX_edens_conv * in_fac; // CU to MeV nm^-3
          my_grey_opacity_params.m1_pars.chi[ig] = chit[i4D];

          // Fill data for anti-heavy neutrinos. We should handle this in a more
          // robust way
          if (ig == 2) {
